...
sys_exofork(void)
{
	...
	struct Env *e;
	int err = 0;
	if((err = env_alloc(&e, curenv->env_id) < 0) < 0){
		return err;
	}
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	if((err = envid2env(srcenvid, &src_env, 1)) < 0){
		return err;
	}
	...
	if((err = envid2env(dstenvid, &dst_env, 1)) < 0){
		return err;
	}
	if((err = page_insert(dst_env->env_pgdir, pp, dstva, perm)) < 0){
		return err;
	}
	return 0;
}
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if((uintptr_t)srcva < UTOP){
		...
		// srcva is not mapped
		if(pte == NULL || (*pte & req_perm) != req_perm){
			return -E_INVAL;
		}
		// srcva is read-only, but perm & PTE_W
		if((perm & PTE_W) && !(*pte & PTE_W)){
			return -E_INVAL;
		}
	}
	...
	if((uintptr_t)srcva < UTOP && (uintptr_t) dstenv->env_ipc_dstva < UTOP){
		if((err = page_insert(dstenv->env_pgdir, pp, dstenv->env_ipc_dstva, perm)) < 0){
			return err;
		}
...
static int sys_net_tx_packet(void *buf, size_t len){
	user_mem_assert(curenv, buf, len, PTE_U);
	return e1000_tx_packet(buf, len);
}

static int sys_net_rx_packet(void *buf, size_t len){
	user_mem_assert(curenv, buf, len, PTE_U | PTE_W);
	return e1000_rx_packet(buf, len);
}
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	case SYS_net_tx_packet:
		return sys_net_tx_packet((void *)a1, a2);
	case SYS_net_rx_packet:
		return sys_net_rx_packet((void *)a1, a2);
	default:
		return -E_INVAL;
		break;
	}
